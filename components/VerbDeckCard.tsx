...
import type { VerbDeckWithCounts } from "@/lib/db";
import { LANGUAGES, type LanguageCode } from "@/lib/db";

export default function VerbDeckCard({ deck }: { deck: VerbDeckWithCounts }) {
    ...
    <div className="bg-surface rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
      <Link href={`/verbs/${deck.id}`}>
        ...
        <p className="text-sm text-text-muted mb-3">
          {langName} → {translationName}
        </p>
        <p className="text-xs text-text-muted">
          {deck.verb_count} {deck.verb_count === 1 ? "verb" : "verbs"}
        </p>
      </Link>
